...
const ManageAllOrderRow = ({ order, index, getData, refetch }) => {
    ...
    return (
        <tr>
            ...
            <td>{orderStatus === 'unpaid' ? <label onClick={() => getData({ _id, name })} for="cancel_order" class="btn modal-button btn-error">Delete Order</label> : orderStatus === 'paid' ? <button onClick={() => handleDeliver(_id)} className="btn btn-md btn-accent w-full">Deliver</button> : <span className='text-xl font-bold text-primary'>Delivered</span>}</td>
        </tr >
    );
};
